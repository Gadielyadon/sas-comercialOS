...
// GET /api/clientes/:id/ventas — historial de ventas del cliente
router.get('/:id/ventas', (req, res) => {
  try {
    const ventas = svc.getHistorialVentas(Number(req.params.id));
    res.json(ventas);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// GET /api/clientes/:id/estadisticas — resumen estadístico del cliente
router.get('/:id/estadisticas', (req, res) => {
  try {
    const stats = svc.getEstadisticasCliente(Number(req.params.id));
    res.json(stats);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
...
// POST /api/clientes/:id/check-limite — verifica si una venta supera el límite
router.post('/:id/check-limite', (req, res) => {
  try {
    const { monto } = req.body;
    if (!monto) return res.status(400).json({ error: 'Falta monto' });
    const result = svc.checkLimite(Number(req.params.id), Number(monto));
    res.json(result);
  } catch(e) { res.status(500).json({ error: e.message }); }
});

// POST /api/clientes/importar — importación masiva
router.post('/importar', (req, res) => {
  try {
    const { rows } = req.body;
    if (!Array.isArray(rows) || !rows.length) return res.status(400).json({ error: 'Sin datos' });
    const result = svc.importMasivo(rows);
    res.json(result);
  } catch(e) { res.status(500).json({ error: e.message }); }
});
